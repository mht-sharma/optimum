...
def preprocess_function(examples, processor, tokenizer):
    ...
    onnx_inputs_decoder = {}
    onnx_inputs_decoder["input_ids"] = [input_ids[:-2]]
    ...
    output["input_ids"] = [input_ids[-2]]
    for i in range(int((len(sess_output_decoder) - 1) / 4)):
        output["past_key_values.{}.decoder.key".format(i)] = sess_output_decoder[i * 2 + 1][0]
        output["past_key_values.{}.decoder.value".format(i)] = sess_output_decoder[i * 2 + 2][0]
        output["past_key_values.{}.encoder.key".format(i)] = sess_output_decoder[i * 2 + 3][0]
        output["past_key_values.{}.encoder.value".format(i)] = sess_output_decoder[i * 2 + 4][0]
